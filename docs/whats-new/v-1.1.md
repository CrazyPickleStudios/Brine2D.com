...
- **`Behavior`** now exposes the same ambient framework properties as `Scene` (`Logger`, `Renderer`, `Input`, `Audio`, `Game`), so no more constructor-injecting these into behaviors by hand.
...
- **`Brine2D.Build`** asset generation is now visible to Visual Studio's Fast Up-to-Date Check, so no more manual `obj`/`bin` cleans after adding a new asset file.
