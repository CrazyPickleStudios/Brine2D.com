...
!!! abstract "In this article"
    `AnimationLayer` setup, `AnimationLayerMask` flags (which `SpriteComponent` properties a layer
    writes), `AnimationLayerBlendMode` (Override vs Additive), priority ordering, per-layer state
    machines and blend trees.
